...
begin
  ...
  -- AUTolog activity (Pipeline v2)
  -- Nota: log_activity_event ya persiste lead_id leyendo metadata.lead_id
  begin
  ...
  exception
    when others then
      -- no bloqueamos el move si falla el log (por seguridad operativa)
      v_event_id := null;
  end;
