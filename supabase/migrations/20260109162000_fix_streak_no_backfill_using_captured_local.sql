-- 20260109162000_fix_streak_no_backfill_using_captured_local.sql
-- Ajustar get_okr_streak_with_grace para NO contar backfill usando captured_local
...
as $$
  ...
  streak_days_cte as (
    with recursive s as (
      ...
      select (s.day_local - interval '1 day')::date as day_local
      from s
      join logged_days ld on ld.day_local = (s.day_local - interval '1 day')::date
    )
