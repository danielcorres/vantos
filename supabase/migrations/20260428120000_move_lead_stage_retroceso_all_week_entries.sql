-- Retroceso: marcar TODAS las entradas forward de la semana hacia la etapa abandonada (no solo la más reciente).
...
comment on function public.move_lead_stage(uuid, uuid, text, timestamptz, boolean) is
  'Mueve un lead entre etapas activas. En retroceso, marca is_retroceso en todas las entradas forward de la semana (MTY) hacia la etapa abandonada. p_only_forward evita retrocesos (calendario).';
