-- Retroceso: solo a la etapa activa inmediatamente anterior (misma regla que la UI).
...
comment on function public.move_lead_stage(uuid, uuid, text, timestamptz, boolean) is
  'Mueve un lead entre etapas activas. Retroceso solo un paso (etapa activa con position máxima menor que la actual). En retroceso válido, marca is_retroceso en entradas forward de la semana (MTY). p_only_forward evita retrocesos (calendario).';
