-- La fila nueva de historial al retroceder debe contar en get_weekly_pipeline_entries (etapa destino).
-- El descuento de la etapa abandonada sigue con el UPDATE que marca is_retroceso en entradas forward de la semana.
...
begin
  ...
  if v_is_backward then
    select max(s.position) into v_prev_position
    from public.pipeline_stages s
    where s.is_active = true
      and s.position < v_from_position;

    if v_prev_position is null or v_prev_position is distinct from v_to_position then
      raise exception 'Retroceso permitido solo a la etapa inmediatamente anterior.';
    end if;
  end if;
  ...
  if v_is_backward then
    ...
    where h.lead_id = p_lead_id
      and h.to_stage_id = v_from_stage_id
      and coalesce(h.is_retroceso, false) = false
      and (coalesce(h.occurred_at, h.moved_at) at time zone 'America/Monterrey')::date >= v_week_start
      and (coalesce(h.occurred_at, h.moved_at) at time zone 'America/Monterrey')::date < v_week_start + 7;
  end if;
...
comment on function public.move_lead_stage(uuid, uuid, text, timestamptz, boolean) is
  'Mueve un lead entre etapas activas. Retroceso solo un paso. En retroceso, anula conteo semanal de entradas forward a la etapa abandonada (is_retroceso en historial previo); la fila nueva hacia la etapa anterior cuenta en productividad (is_retroceso false). p_only_forward evita retrocesos (calendario).';
